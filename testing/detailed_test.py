...
def detailed_visualization_step(points_dict, title, save_path, step_num):
    ...
    for i, (name, points) in enumerate(points_dict.items()):
        ...
        if len(points) > 1000:
            ...
            ax.set_title(f"{name}\n({len(points)} points, showing 1000)")
        else:
            viz_points = points
            ax.set_title(f"{name}\n({len(points)} points)")
    ...
    # If we have multiple point sets, create comparison plots
    if n_sets >= 2:
        # Overlay plot
        ax_overlay = fig.add_subplot(2, 3, n_sets + 1, projection='3d')
        ...
        for i, (name, points) in enumerate(points_dict.items()):
            ...
            ax_overlay.scatter(viz_points[:, 0], viz_points[:, 1], viz_points[:, 2],
                               c=color, marker=marker, s=15, alpha=0.6, label=name)

        ax_overlay.set_title("Overlay Comparison")
        ax_overlay.legend()
        ax_overlay.set_xlabel('X')
        ax_overlay.set_ylabel('Y')
        ax_overlay.set_zlabel('Z')

    plt.suptitle(f"Step {step_num}: {title}", fontsize=16, fontweight='bold')
    plt.tight_layout()
