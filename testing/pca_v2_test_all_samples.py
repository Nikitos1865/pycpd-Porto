...
def create_results_visualization(specimen_name, deca_mean_source, aligned_test_target, deca_mean_transformed_pca, deca_mean_transformed_pca_v2,
    ...
    # PCA_V2 transformed
    ax.scatter(deca_mean_transformed_pca_v2[:, 0], deca_mean_transformed_pca_v2[:, 1], deca_mean_transformed_pca_v2[:, 2],
...
def process_specimen(specimen_file, skull_source, mean_shape, U_reduced, eigenvalues, deca_mean_source, results_dir):
    ...
    # Step 1: Run PCA-V2 CPD
    print(f"Running PCA-V2 CPD for {specimen_name}...")
    ...
    pca_method_difference = ((traditional_rmse - pca_rmse) / traditional_rmse) * 100 if traditional_rmse > pca_rmse else ((pca_rmse - traditional_rmse) / pca_rmse) * 100
    pca_better_method = 'PCA' if pca_rmse < traditional_rmse else 'Traditional'

    pca_v2_method_difference = ((traditional_rmse - pca_v2_rmse) / traditional_rmse) * 100 if traditional_rmse > pca_v2_rmse else ((pca_v2_rmse - traditional_rmse) / pca_v2_rmse) * 100
    ...
    print(f"PCA-V2-CPD RMSE: {pca_v2_rmse:.6f} (Improvement: {pca_v2_improvement:.2f}%)")
    print(f"Traditional CPD RMSE: {traditional_rmse:.6f} (Improvement: {traditional_improvement:.2f}%)")
    print(f"Difference between PCA and traditional methods: {abs(pca_method_difference):.2f}% ({pca_better_method} better)")
    print(f"Difference between PCA-V2 and traditional methods: {abs(pca_v2_method_difference):.2f}% ({pca_v2_better_method} better)")
...
def main():
    ...
    if all_results:
        ...
        print(f"Average PCA-CPD-V2 RMSE: {df['pca_v2_rmse'].mean():.6f}")
        ...
        plt.bar([i + width for i in x], df_sorted['pca_v2_rmse'], width, label='PCA-V2-CPD RMSE', color='purple')
        plt.bar([i + 2*width for i in x], df_sorted['traditional_rmse'], width, label='Traditional CPD RMSE',
                color='blue')
        ...
        plt.bar([i for i in x],
                df_sorted['pca_v2_improvement'],
                width=width,
                label='PCA-V2-CPD Improvement',
                color='purple')
